...
import { WEEKDAYS, type AvailabilityRow } from "../../../egitmen/(panel)/musaitlik/types";
import { addInstructorAvailability, removeInstructorAvailability } from "./availability-actions";
...
export function AvailabilityEditor({
  ...
  const [result, formAction] = useActionState(
    addInstructorAvailability.bind(null, instructorId),
    null
  );
              ...
              <ul className="flex flex-col gap-1.5">
                {day.slots.map((slot) => (
                  <li key={slot.id} className="flex items-center justify-between text-sm">
                    ...
                    <DeleteButton
                      action={removeInstructorAvailability.bind(null, instructorId, slot.id)}
