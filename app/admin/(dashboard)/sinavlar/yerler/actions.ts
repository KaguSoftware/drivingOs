...
export async function createExamPlace(
  ...
  redirect(withToast("/admin/sinavlar?tab=places", "Sınav yeri eklendi"));
}
...
export async function updateExamPlace(
  ...
  revalidatePath("/admin/sinavlar");
  redirect(withToast("/admin/sinavlar?tab=places", "Değişiklikler kaydedildi"));
}
...
export async function deleteExamPlace(id: string): Promise<void> {
  ...
  revalidatePath("/admin/sinavlar");
  redirect("/admin/sinavlar?tab=places");
}
